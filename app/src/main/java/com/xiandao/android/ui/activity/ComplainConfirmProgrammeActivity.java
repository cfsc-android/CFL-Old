...
import com.xiandao.android.ui.fragment.ComplainComfirmProgrammeFragment;
...
/**
 * 此类描述的是:投诉--是否接受解决方案activity
 *
 * @author TanYong
 *         create at 2017/6/2 10:05
 */
@ContentView(R.layout.activity_complain_detail)
public class ComplainConfirmProgrammeActivity extends BaseActivity implements View.OnClickListener {
     ...
     * 两个Fragment（页面）
     */
    private ComplainComfirmProgrammeFragment comfirmProgrammeFragment;
    private ComplainPlanFragment complainPlanFragment;
    ...
    private void init() {
        ...
        comfirmProgrammeFragment = new ComplainComfirmProgrammeFragment();
        complainPlanFragment = new ComplainPlanFragment();

        Bundle bundleProgramme = new Bundle();
        bundleProgramme.putString("complainId", complainId);
        bundleProgramme.putString("taskId", taskId);
        bundleProgramme.putString("piid", piid);
        bundleProgramme.putBoolean("isDetail", isDetail);
        Bundle bundleJindu = new Bundle();
        bundleJindu.putString("complainId", complainId);

        comfirmProgrammeFragment.setArguments(bundleProgramme);
        complainPlanFragment.setArguments(bundleJindu);

        fragmentList.add(comfirmProgrammeFragment);
        ...
        vpContent.setAdapter(new ComplainConfirmProgrammeActivity.MyFrageStatePagerAdapter(getSupportFragmentManager()));
    }
