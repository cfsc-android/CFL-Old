...
@ContentView(R.layout.activity_join_in)
public class JoinInActivity extends BaseActivity {
    @ViewInject(R.id.tv_title_name)
    private TextView tvTitleName;
    @ViewInject(R.id.lv_join_in)
    private ListView lv_join_in;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvTitleName.setText("入伙");
        mNoticeListAdapter=new NoticeListAdapter(this,noticeList);
        lv_join_in.setAdapter(mNoticeListAdapter);
        lv_join_in.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                NoticeEntity selectNotice = noticeList.get(position);
                Bundle bundle = new Bundle();
                bundle.putString("title","入伙");
                ...
                new HttpUtils.DataCallBack() {
                    ...
                    public void callBack(Object o) {
                        if (o != null) {
                            ...
                            if (noticeListEntity != null && noticeListEntity.getResultCode().equals("0")) {
                                ...
                                if (resultNoticeList != null && resultNoticeList.size() > 0) {
                                    for (int i = 0; i < resultNoticeList.size(); i++) {
                                        if(resultNoticeList.get(i).getNoticeType()==4){
                            ...
                            NetUtil.toNetworkSetting(JoinInActivity.this);
                        }
                    }
                });
    }
