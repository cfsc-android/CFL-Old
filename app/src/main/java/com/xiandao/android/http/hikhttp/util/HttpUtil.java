...
import com.xiandao.android.http.hikhttp.Response;
...
//        ((Map)headers).put("Content-Type", "application/x-www-form-urlencoded;charset=UTF-8");
        Map<String, String> _headers = initialBasicHeader("POST", path, (Map)headers, querys, bodys, signHeaderPrefixList, appKey, appSecret);
        HttpClient httpClient = wrapClient(host);
        httpClient.getParams().setParameter("http.connection.timeout", getTimeout(connectTimeout));
        HttpPost post = new HttpPost(initUrl(host, path, querys));
        Iterator var11 = _headers.entrySet().iterator();

        while(var11.hasNext()) {
            Map.Entry<String, String> e = (Map.Entry)var11.next();
            System.out.println(e.getKey()+"========"+MessageDigestUtil.utf8ToIso88591(e.getValue()));
